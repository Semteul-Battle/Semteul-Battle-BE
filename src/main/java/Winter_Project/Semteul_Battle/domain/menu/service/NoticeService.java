...
import Winter_Project.Semteul_Battle.domain.menu.dto.notice.*;
...
MenuNotice notice = menuNoticeRepository.findById(noticeId)
                .orElseThrow(() -> new EmptyResultDataAccessException("???????????⑤９????????耀붾굝????????????????源낆┰?????????곸죩.", 1));

        // ???????????⑤９????????????곌떽釉붾?????????????????????????????????耀붾굝????? ??饔낅떽???????????轅붽틓??????
if (!notice.getUsers().getLoginId().equals(loginId)) {
            throw new ResponseStatusException(HttpStatus.FORBIDDEN, "???????????⑤９????????????곌떽釉붾?????????????????????????????源낆┰?????????곸죩.");
        }
